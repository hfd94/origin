...
func (cls *Cluster) ReadClusterConfig(path string) (*NodeInfoList, error) {
	c := &NodeInfoList{}
	...
	if ext == ".json" {
		...
		return nil, fmt.Errorf("the configuration file type is incorrect")
	}
...
func (cls *Cluster) readServiceConfig(filepath string, isJson bool) (interface{}, map[string]interface{}, map[string]map[string]interface{}, error) {
...
func (cls *Cluster) readLocalClusterConfig(nodeId string) (DiscoveryInfo, []NodeInfo, RpcMode, error) {
	...
	clusterCfgPath := configDir //strings.TrimRight(configDir, "/") + "/cluster"
...
func (cls *Cluster) readLocalService(localNodeId string) error {
	...
	for _, f := range fileInfoList {
		...
		currGlobalCfg, serviceConfig, mapNodeService, err := cls.readServiceConfig(filePath, ext == ".json")
		if err != nil {
			continue
		}
